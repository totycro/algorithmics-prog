...
For small values of  $u_{max}$, this constraint tightens the selection of the \ilc{u} values, since we know that the sum of all u values can be at most $\sum_1^k{k}$
\begin{equation}\label{eq:mztMaxU}
	\sum_{\vertex \in V}{\ilc{u[\vertex]}} \leq \frac{k * (k+1)}{2}
\end{equation}

The value of $u_{max}$ was also subject to a strengthening or relaxing. The original proposal has $u_{max}$ set to the number of nodes, which makes sense for a TSP that is a path. So we tried n. But in our case, we only have k nodes, so k is a more natural and tighter candidate. But we also tried larger number like $n*k$. Our idea was, that the actual u value does not matter, but only the ordering is important (a node "closer" to the root has to have a lower \ilc{u} value). So a larger $u_{max}$ value gives the solver more freedom to find a suitable choice.
...
The biggest impact on the performance was the introduction of \eqref{eq:StrAll}. It was initially thought as a solely strengthening constraint for all three models, but in the case of MTZ, where \eqref{eq:mztUnused} insures the same property (no outgoing edges without incoming edges), replacing the constraint resulted in a large performance boost. It had no such effect on SCF and MCF, we think that this is already implied equally closde by the flow formulations.


The other strengthening constraint usually did not yield in significant performance changes, which seems reasonable since they only tighten the model for a few constraints. 
